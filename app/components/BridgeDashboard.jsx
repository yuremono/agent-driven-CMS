    ...
    <main className={shellClass}>
      <section className={heroClass}>
        <div className="grid gap-4">
          ...
          <h1 className={`${titleClass} text-[clamp(2.4rem,4.2vw,4.8rem)] leading-[0.95]`}>
        ...
        <aside className={`${panelClass} grid gap-4 lg:sticky lg:top-5`}>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {status.accountEmail ? <MetaChip>account: {status.accountEmail}</MetaChip> : null}
          </div>
            ...
            {supportsRateLimits && rateLimitSummary ? (
              <>
                ...
                <p className={`m-0 text-sm leading-7 ${mutedClass}`}>
                  {formatPercent(rateLimitSummary.usedPercent)} used ·{" "}
                  {formatWindow(rateLimitSummary.windowDurationMins)} window · reset{" "}
      ...
      <section className={panelClass}>
        ...
        <form className="mt-4 grid gap-3" onSubmit={handleSubmit}>
          <textarea
            ...
            className="min-h-[112px] w-full resize-y rounded-2xl border border-[rgba(38,27,18,0.1)] bg-white/75 px-4 py-3 text-[#1d1712] outline-none placeholder:text-[rgba(104,95,85,0.58)]"
            value={input}
        ...
        <div className="mt-4 grid gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {provider === "claude" ? (
              ...
              <span className={`text-sm ${mutedClass}`}>routes: (not exposed)</span>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6">
        <article className={panelClass}>
          <PanelHeader title="Agent Reply" description="直近の応答をそのまま追えます。" />
          <div className="mt-4 grid gap-3 rounded-[22px] bg-white/65 p-4 ring-1 ring-[rgba(38,27,18,0.08)]">
            <p className={`m-0 text-sm ${mutedClass}`}>turn: {currentTurnId ?? "none"}</p>
          ...
          <div className="mt-4 grid gap-4">
            {!supportsApprovals ? (
              ...
              approvals.map((request) => {
                const decisions = decisionOptions(request.method);
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                      <strong className="text-[#1d1712]">{requestTitle(request.method)}</strong>
                      <span className={`text-sm ${mutedClass}`}>
                        request: {String(request.requestId)}
                      </span>
                    </div>
                    {params.reason ? (
                      <p className={`m-0 leading-7 ${mutedClass}`}>{params.reason}</p>
                    ) : null}
                    {params.command ? (
                      <p className={`m-0 grid gap-1 text-sm leading-7 ${mutedClass}`}>
                        <span>cmd</span>
                    ...
                    {Array.isArray(params.changes) && params.changes.length > 0 ? (
                      ...
                      {decisions.length > 0 ? (
                        decisions.map((decision) => (
                          <button
                            ...
                            onClick={() => handleApproval(request, decision)}
                          >
                            {decision}
                          </button>
                        ))
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <article className={panelClass}>
          <PanelHeader title="Events" description="SSE で届くイベントを最新順に並べます。" />
          <div className="mt-4 grid max-h-[360px] gap-2 overflow-auto">
            {events.length === 0 ? (
              <p className={`m-0 text-sm leading-7 ${mutedClass}`}>まだイベントはありません。</p>
            ) : (
